...
export async function generateEmployeeCard(data: EmployeeCardData): Promise<Buffer> {
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            // Header sections
            doc.rect(0, 0, CARD_WIDTH, 70).fill(COLORS.primary)

            // Employee Tag Tile
            doc.font("Times-Bold").fontSize(18).fillColor(COLORS.white).text("EMPLOYEE TAG", 0, 20, {
                width: CARD_WIDTH,
                align: "center"
            })

            // Facility Name
            doc.font("Times-Roman").fontSize(14).fillColor(COLORS.white).text(data.facilityName, 0, 45, {
                width: CARD_WIDTH,
                align: "center",
            })
            ...
            if (data.imageBuffer) {
                ...
                doc.circle(photoX + photoSize / 2, photoY + photoSize / 2, photoSize / 2).lineWidth(2).stroke(COLORS.primary);
            } else {
            ...
            doc
                .rect(0, CARD_HEIGHT - 20, CARD_WIDTH, 20)
                .fill(COLORS.primary);
